...
function asstSide(s) { return String(s).toLowerCase() === "sell" ? { sell: null } : { buy: null }; }
...
const ASSISTANT_ACTIONS = {
  placeMarketOrder: {
    ...
    summary: (a) => `Spot MARKET ${String(a.side).toUpperCase()} ${a.quantity} on ${a.marketId} · max slippage ${(Number(a.maxSlippage) * 100).toFixed(1)}%`,
  ...
  placeLimitOrder: {
    ...
    summary: (a) => `Spot LIMIT ${String(a.side).toUpperCase()} ${a.quantity} on ${a.marketId} @ ${a.price}`,
  ...
  openPosition: {
    ...
    summary: (a) => `OPEN ${String(a.side).toLowerCase() === "sell" ? "SHORT" : "LONG"} ${a.size} on ${a.marketId} in pool #${a.poolId}${a.limitPrice ? ` (limit ${a.limitPrice})` : ""}`,
...
function assistantSystemPrompt(schemaText) {
  ...
  const actionLines = Object.values(ASSISTANT_ACTIONS).map((v) => "  - " + v.sig).join("\n");
...
- The \`order\` entity IS the live order book (open + partially-filled resting orders, including the AMM's own quotes). You can gauge depth and estimate the approximate slippage of a market order by querying the relevant side sorted by price and walking it: a BUY consumes the lowest-priced \`sell\` orders (asks, orderBy price asc); a SELL hits the highest-priced \`buy\` orders (bids, orderBy price desc). Accumulate quantity×price across rows until the order's notional (e.g. $1000) is filled, take the volume-weighted average fill price, and compare it to the best (first) price — that deviation is the approximate slippage. Use \`quantity\` as the resting size per row.
...
function assistantFullPrompt(schemaText) {
  const convo = _asstTurns.map((t) => t.role.toUpperCase() + ": " + t.text).join("\n\n");
  return assistantSystemPrompt(schemaText) + "\n\n=== CONVERSATION ===\n" + convo + "\n\nASSISTANT (one JSON object only):";
}
    ...
    for (let step = 0; step < ASSIST_MAX_STEPS; step++) {
      ...
      if (obj.type === "query") {
        ...
        try {
          ...
          resultText = asstJson(objs) + (res.hasMore ? " (truncated)" : "");
        } catch (e) {
      ...
      if (obj.type === "candles") {
        ...
        const ALLOWED_IVS = [60000, 300000, 900000, 3600000, 14400000, 43200000, 86400000, 604800000];
        let resultText;
        try {
      ...
      if (obj.type === "action") {
        ...
        const ok = await assistantConfirm(obj.method, obj.args, act.summary(obj.args || {}));
        ...
        try {
          ...
          outcome = ("ok" in res) ? ("OK" + (typeof res.ok === "bigint" ? " (#" + Number(res.ok) + ")" : "")) : ("ERR " + res.err);
        ...
        try { if (typeof refreshAccountData === "function") refreshAccountData(); } catch (_) {}
        continue;
      }
      if (obj.type === "refused") {
